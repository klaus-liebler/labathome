...
import { existsBoardSpecificPath, IApplicationInfo, strInterpolator, writeBoardSpecificFileCreateDirLazy } from "./gulpfile_utils";
...
export async function createSpeech(ai: IApplicationInfo){
  var boardSounds=ai.board.board_settings?.speech as Array<FilenameAndSsml>|undefined;
  var boardTypeSounds=ai.board.board_type_settings?.speech as Array<FilenameAndSsml>|undefined;
  var appSounds=ai.app_settings?.speech as Array<FilenameAndSsml>|undefined;
  var sounds=new Array<FilenameAndSsml>().concat(boardSounds??[], boardTypeSounds??[], appSounds??[]);
  
  const client = new tts.TextToSpeechClient();
  // Construct the request
  for (const e of sounds) {
    if (existsBoardSpecificPath(ai.board, P.SOUNDS_DE_SUBDIR, e.name+".mp3")){
      continue;
    }
    ...
    writeBoardSpecificFileCreateDirLazy(ai.board,P.SOUNDS_DE_SUBDIR, e.name+".mp3", response.audioContent as Uint8Array);
  };
}