...
import { Html, HtmlAsFirstChild } from '@klaus-liebler/web-components/typescript/utils/common';
import { IAppManagement } from '@klaus-liebler/web-components/typescript/utils/interfaces';
import { html } from 'lit-html';
import { createRef, ref, Ref } from 'lit-html/directives/ref.js';
import { heaterexperiment } from "@generated/wsprotocol_ts/ws-protocol";
const Mode = heaterexperiment.Mode;
type Mode = heaterexperiment.Mode;
...
export class HeaterExperimentController extends ScreenController {
    ...
    private mode: Mode = Mode.FUNCTION_BLOCK;
    private seconds = 0;
    ...
    public Template = () => html`
     <form>
            ...
            <div class="mode_toggler">
              <label>
                <input type="radio" name="heaterexperiment_mode" @click=${() => {this.onModeChange(Mode.FUNCTION_BLOCK)}} checked />
              ...
              <label>
                <input type="radio" name="heaterexperiment_mode" @click=${() => {this.onModeChange(Mode.OPEN_LOOP)}}  />
              ...
              <label>
                <input type="radio" name="heaterexperiment_mode" @click=${() => {this.onModeChange(Mode.CLOSED_LOOP)}}  />
    ...
    <form ${ref(this.frmClosedloop)}>
            ...
            <div class="range-wrap heaterexperiment heaterexperiment_closedloopctrl">
              <input type="range" ${ref(this.inputFanCL)} @input=${(ev:InputEvent)=>{this.setMyBubble(ev.currentTarget as HTMLInputElement)}} min="0" max="100" value="0" />
              <output class="bubble"></output>
            </div>
...
public OnFirstStart(): void {
        ...
        this.onModeChange(0);
        // Alle Slider auf 0 initialisieren und die Wert-Labels (bubble) daran ausrichten
        for (const r of [this.inputSetpointHeater, this.inputFanOL, this.inputSetpointTemperature, this.inputFanCL]) {
            r.value!.value = "0";
            this.setMyBubble(r.value!);
        }
    ...
    public OnCreate() {
        this.appManagement.RegisterNamespace(this, heaterexperiment.NAMESPACE_ID);
    }


    public OnMessage(namespaceId: number, messageTypeId: number, view: DataView): void {
        if(namespaceId!=heaterexperiment.NAMESPACE_ID){
            console.error("heater experiment controller namespace problem: " + namespaceId);
            return;
        }
        if(messageTypeId!=heaterexperiment.ResponseHeater.TYPE_ID){
            console.error("heater experiment controller: Unknown message type " + messageTypeId);
            return
        }
        const r = heaterexperiment.ResponseHeater.decode(view, 0);
    ...
    private sendAndReceive() {
        const bytes = heaterexperiment.RequestHeater.encode({
            requestId: 0,
            mode: this.mode,
            heaterPowerPercent: this.inputSetpointHeater.value!.valueAsNumber,
            setpointTemperatureDegrees: this.inputSetpointTemperature.value!.valueAsNumber,
            fanSpeedPercent: this.mode==Mode.OPEN_LOOP?this.inputFanOL.value!.valueAsNumber:this.inputFanCL.value!.valueAsNumber,
            kp: this.inputKP.value!.valueAsNumber,
            tn: this.inputTN.value!.valueAsNumber,
            tv: this.inputTV.value!.valueAsNumber,
            heaterPowerWorkingPointPercent: this.inputWP.value!.valueAsNumber,
            regulatorReset: this.inputReset.value!.checked ? true : false,
        });
        this.appManagement.SendFrame(heaterexperiment.NAMESPACE_ID, bytes, 0);
    }
